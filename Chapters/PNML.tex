...
\label{Chapter: Petri net representation for subnets and hiding support. PNML} % Change X to a consecutive number; for referencing this chapter elsewhere, use \ref{Chapter3}
...
As we have seen before (in chapter \ref{Chapter: Private_information_inPetri_nets.Subnets}), we can reorder rows and columns and define subnets
in the matrix.
...
or this other one, grouping the places and transitions of the hidden subnet as seen in chapter \ref{Chapter: Private_information_inPetri_nets.Subnets}
\[
\kbordermatrix{
   &  _ & \vline &t_3 & t_4\\
...
and the outside of the subnet completely. Taking advantage of the process described in chapter \ref{Chapter: Private_information_inPetri_nets.Subnets}, I have to extract the front-end from this subnet.
...
\begin{itemize}
\item \texttt{\textless subnet\textgreater} for delimitate the scope (places
and transitions) of the subnet
\item 
\textless interface\textgreater\ for define the gates to enter or leave the
subnet. Each arc entering or leaving the subnet is associated to a specific
gate. Each gate is represented with the tag \texttt{\textless gate\textgreater}\ and
...
\item \texttt{\textless content\textgreater}\ where the places and transitions
