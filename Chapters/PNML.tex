...
Once this structure is defined, I am going to explain the next stage, which
...
Transitions are represented with the tag \texttt{<transition>}. Except the initial marking, it is really similar to a place. This is an example of a transition in PNML: 

\begin{lstlisting}[label=pnml_transition,caption=PNML representation
for transitions]
<transition id="t1">
  <name>
...
Arcs are represented with the tag \texttt{<arc>}. Arcs
must have a source and a target, which are defined by the attributes \texttt{source} and \texttt{target} that have to point to a transition and a place, identified
by their \texttt{id}. Furthermore, the arc weight can be fixed by the tag \texttt{<inscription>}.
...
\begin{lstlisting}[label=pnml_arc_general,caption=General PNML representation
for arcs with arbitrary weight]
...
\begin{lstlisting}[label=pnml_arc_weight_1_1,caption=PNML representation
for arcs of weight 1]
<arc id="a1" source="p1" target="t1">
</arc>
\end{lstlisting}

or the same
\begin{lstlisting}[label=pnml_arc_weight_1_2,caption=PNML representation
for arcs of weight 1]
<arc id="a1" source="p1" target="t1"/>
\end{lstlisting}
...
\begin{lstlisting}[label=pnml_complete_representation,caption=Complete PNML representation
...
For clarity, I am going to obviate several options. I am not going to draw the graphic information and the names, but the id. So this last example is as follows:

\[
\includegraphics[width=0.11\textwidth]{Figures/PNML-RdPBasica.eps}
\]

\begin{lstlisting}[label=pnml_basic_representation,caption=Basic PNML representation
for a basic Petri net]
<?xml version="1.0" encoding="utf-8"?>
<pnml>
  <net id="myNet" type="http://www.pnml.org/version-2009/grammar/ptnet">
...
this criterion, the listing \ref{pnml_basic_representation} changes to:

\begin{lstlisting}[label=pnml_simplified_representation,caption=Simplified PNML representation
for a basic Petri net]
<place id="p1">
  <initialMarking>
    <text> 2 </text>
...
the PNML content that represents this Petri net is as follows: \note{This PNML content is in two columns for space and clarity reasons.}
\begin{multicols}{2}
...
As we can see, the size of the PNML representation for a relative small Petri
net, is significant big, even the graphics content
is deleted. But this is not a problem because this PNML representation is
oriented to computer processing.
...
\]
\rule{35em}{0.5pt}
 \caption{Three Petri net examples}
 \label{fig:Petri_net_examples} 
\end{figure}
Note that they have common or very similar parts. For example, a) and b)
...
to represent subnets inside a concrete Petri net.

Let's take simple Petri net of the figure \ref{fig:PNML_subnet_to_representRdPSubred}. It will serve us to explain
the method to achieve a subnet representation and the PNML extension associated
to it parting from a determinate Petri net:
...
the id, of course)
but
it has embedded the gates inside of it. These gates are represented by \texttt{<gate>}. This tag has two new attributes: \texttt{action} and \texttt{type}.
...
whether the gate is an input or an output gate. The other attribute,
...
the tag \texttt{<inscription>} embedded in the tag \texttt{<gate>}. It has the weight of the arc associated. In the same way that with arcs, if they
have weight 1 this tag can be obviated. 
...
\]
\rule{35em}{0.5pt}
 \caption{Three Petri subnet examples}
...
there is a little difference with a). There are arcs from one output
