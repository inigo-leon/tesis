...
A subnet corresponds \cite{G-Silva1985,HID-Inigo2011MT}, in terms of matrices, with the resulting submatrix keeping only the rows and columns corresponding to places and transitions of the selected subnet.
...
two arcs.
\item It does not matter the number of places and transitions that are chosen
for the subnet, as long as they are not empty sets and have, at least one
place and one transition. If it had only a place, it is a link place (there
is an isolated row in the incidence matrix) and, if it had only a transition
it would be a link transition (there is an isolated column in the incidence
matrix).  
\end{enumerate}
...
\subsection{Subnetting: splitting a net into subnets}
...
\begin{itemize}
  ...
  \item $\forall i, j | 1\leqslant i, j \leqslant k \Rightarrow\ N_i \cap N_j = \emptyset$ (pairwise disjointed)
\end{itemize}
...
\subsubsection{Disjoint subnets}
...
\begin{figure}
\[
\begin{matrix}\includegraphics[width=0.3\textwidth]{Figures/macrolugar.eps}\end{matrix}
\ \ \ \ \ \ 
C=
\kbordermatrix{
...
\centering
\[
\begin{matrix}\includegraphics[width=0.3\textwidth]{Figures/macrotransicion.eps}\end{matrix}
\ \ \ \ \ \ 
C=
\kbordermatrix{
   & t_1 & \vline & t_2 & t_3 & t_4\\
...
%//Estos cuatro siguientes TODO no son estrictamente necesarios. Sólo si da tiempo
...
%//misma red, con subredes interrelacionadas por esta caracteristica. Una
%//subred es macrolugar sobre otra si y solo si esta ultima es macroplace sobre la primera. Comprobarlo y demostrarlo en su caso
...
\subsubsection{Link transition and link place}

Once done a partition of a net, maybe we want to have places or transitions that are not included in other subnets. In these cases we talk about link
transitions or link places.
In those cases, the matrix has isolated columns
or rows respectively.

\begin{example}
Let's take again the Petri net defined in example \ref{ej:petri_net} and
...
Now we are going to define two subnets in this way:\begin{itemize}
\item $N1=p_1, p_2, t_1$
\item $N2=p_3, p_4, p_5, t_3, t_4$
\end{itemize}

As we can see, $t_2$ is outside both nets so it is a link transition
 

\begin{figure}[htbp]
\centering
\[
\begin{matrix}\includegraphics[width=0.4\textwidth]{Figures/link_transition.eps}\end{matrix}
\ \ \ \ \ \ 
C=
\kbordermatrix{
   & t_1 & \vline & t_3 & t_4 & & \color{blue}t_2\\
p_1& -1 & \vline   &  0  &  1 & |& \color{blue}1\\
p_2&  1 & \vline   &  1  &  0 & |& \color{blue}-1\\
\cline{1-5}
p_3&  0 & \vline   & -1  &  0 & | & \color{blue}1\\
p_4&  0 & \vline   &  1  & -1 & |& \color{blue}0\\
p_5&  0 & \vline   &  0  &  1 & |& \color{blue}0\\
}
\]
\rule{35em}{0.5pt}
\caption{Link transition}
\label{fig:link_transition}
\end{figure}

In the matrix we can see an isolated column $t_2$.

Is the same way, if we want $p_3$ to be outside, instead of $t_1$, it is
a place transition an we have
this other figure:

\begin{figure}[htbp]
\centering
\[
\begin{matrix}\includegraphics[width=0.4\textwidth]{Figures/link_place.eps}\end{matrix}
\ \ \ \ \ \ 
C=
\kbordermatrix{
   & t_1 & \vline & t_2 & t_3 & t_4\\
...
p_5&  0 & \vline   & 0 &  0  &  1\\
\cdashline{2-6}
\color{blue}p_3&  \color{blue}0 &    & \color{blue}1 & \color{blue}-1  &  \color{blue}0\\
}
\]
\rule{35em}{0.5pt}
\caption{Link place}
\label{fig:link_place}
\end{figure}

In the matrix we can see an isolated row $p_3$.

In general, link transitions and link places can be seen as subnets with
one only element.
\end{example}
...
\begin{note}
As my goal is to hide one subnet, link transitions and link places can be
seen as elements of the visible subnet, without loss of generality.
So in the next sections the existence of these kind of elements is not going to be contemplated.
\end{note}
...
\centering
\[
 \begin{matrix} \includegraphics[width=0.4\textwidth]{Figures/NodosEntradaSalidaV.eps}\end{matrix}
  \ \ \ \ \ 
C=
\kbordermatrix{
...
\centering
\[
 \begin{matrix} \includegraphics[width=0.6\textwidth]{Figures/Front-endEnglish.eps}\end{matrix}
\]
\rule{35em}{0.5pt}
 \caption{Net Front-end}
 \label{fig:front-end} 
\end{figure}
...
So, any subnet with the same front-end could replace this subnet in the original
net (attachable subnet) even though we know nothing about the internal structure
of the subnet, because it is hidden.
